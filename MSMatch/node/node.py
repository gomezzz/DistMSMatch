...
class BaseNode:
    ...
    def __init__(
        ...
        self.device = "cuda:{}".format(self.rank % torch.cuda.device_count()) if torch.cuda.is_available() else "cpu"
...
class PaseosNode(BaseNode):
    def __init__(
        ...
        self.tx_duration = model_size_kb / self.local_actor.communication_devices['link'].bandwidth_in_kbps
    ...
    def exchange_models_and_aggregate(self, comm, verbose=False):
        ...
        if len(self.ranks_in_lineofsight) > 0:
            ...
            for i in self.ranks_in_lineofsight:
                ...
                recv_requests.append(comm.irecv(buf=(self.model_size+50), source=i, tag=int(str(i) + str(self.rank))))
                send_requests.append(
            ...
            for recv_request in recv_requests:
                ...
                actor_models.append(bytestream_to_statedict(other_actor_model_bytearray, self.model.train_model))
    ...
    def advance_time(self, timestep):
        self.paseos.advance_time(timestep, 
