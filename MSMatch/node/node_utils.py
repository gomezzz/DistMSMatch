...
def bytestream_to_statedict(bytestream_parameters, model_template):
    ...
    params_dict = zip(model_template.state_dict().keys(), parameters)  # relate each ndarray to a layer
    state_dict = OrderedDict({k: torch.tensor(v) for k, v in params_dict})  # make a dictionary of tensors
    return state_dict
