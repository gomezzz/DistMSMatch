...
class Namespace:
    ...
    async def launch(self):
        ...
        await self.node._update_connections(self, connected)
        if ex:
            raise RuntimeError(
    ...
    async def connect_to(self, peer: Identity) -> bool:
        ...
        """
        is_talking = self.operating_mode == OperatingMode.LEECHING or self.operating_mode == OperatingMode.OFFLINE
        if is_talking or not self.peer_filter(peer):
            return False
    ...
    async def wait_for_peers(
        ...
        * If you pass one or more `Identity` objects,
        ...
        this will match on all immutable attributes of the `Identity` objects.
        ...
        * If you pass an integer,
        ...
        if isinstance(peers, int):
            try:
                peers = await asyncio.wait_for(self._wait_for_n_peers(peers), timeout=timeout)
    ...
    async def recv(self, timeout: Optional[float] = None) -> Tuple[Identity, Sequence[bytes]]:
        ...
        try:
            ...
            peer = rx[0]
            if peer is not None:
                await self.node._update_connections(
                    self, [peer]
                )  # make sure peer identity is updated with fresh metadata
            return rx
...
class Node:
    ...
    def current_namespaces(self):
        """
        All the namespaces the node belongs to

        Returns:
            list[str]: List of all namespaces the node is part of
    ...
    def get_connections(self, name):
        """
        Gets the virtual connections established within the namespace,
        i.e., the connected peers that satisfy the peer_filter
        ...
        if (self._namespaces.get(name)) is not None:
            # the list can change due to the discovery_engine calling _on_update so must make a deep copy
            if self._peers_by_namespace[name] is not None:
                return copy.deepcopy(self._peers_by_namespace[name])
            else:
                return []
        else:
            logger.debug(f"namespace '{name}' unknown")

    async def _update_connections(self, ns: Namespace, peers: Sequence[Identity]) -> None:
        for peer in peers:
            valid_connection = ns.peer_filter(peer)
            already_connected = peer in self._virtual_tx_connections[(peer.host, peer.port)]
            if valid_connection and not already_connected:
                self._virtual_tx_connections[(peer.host, peer.port)].add(peer)
            elif not valid_connection and already_connected:
    ...
    async def _on_appear(self, peer: Identity) -> None:
        ...
        if ns is not None:
            ...
            if connected:
                ...
                cl.debug(f"'{peer.id}' filtered out")

    async def _on_update(self, peer: Identity) -> bool:
        cl = logger.bind(peer=attrs.asdict(peer))
        ...
        if ns is not None:
            valid_connection = ns.peer_filter(peer)
            already_connected = peer in self._virtual_tx_connections[(peer.host, peer.port)]
            if already_connected and not valid_connection:
                await self._disconnect_from(peer)
                cl.debug(f"peer '{peer.id}' is filtered out")
            elif not already_connected and valid_connection:
                await self._on_appear(peer)
