...
class DiscoveryEngine:
    ...
    def update_metadata(self, identity: Identity, metadata: Dict) -> None:
        """
        Update the metadata of an identity from the list of valid
        identities for the current node.
        ...
        """
        self._identities[(identity.id, identity.namespace)].metadata = metadata

    async def start(
        self,
        on_appear: Callable[[Identity], Awaitable[None]],
        on_update: Callable[[Identity], Awaitable[None]],
        ...
        Arguments:
            on_appear (callback):
                A callback to call when a peer appears on the scatterbrained network.
        ...
        async def handle_peer_heartbeat(data):
            ...
            if (peer.id, peer.namespace) in self._identities:
                ...
                self._last_seen[peer] = datetime.now()
                await on_update(peer)

                # Update metadata by removing old peer instance and adding current instance
                self._peers.remove(peer)  # note that this is the peer with the old metadata
                self._peers.add(peer)  # peer with new metadata
